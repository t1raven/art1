...
$reporter = isset($_POST['reporter']) ? $_POST['reporter'] : 'art1';
...
$arrWaterStatusVideo =  isset($_POST['water_status_video']) ? $_POST['water_status_video'] : null;
...
$reporter = (empty($reporter))? 'art1' : $reporter ;
...
//if($newsCate1 == 1){
	...
	if ($imgMainFile['name']) {
		...
		if($newsCate1 == 1){
			LIB_FILE::ThumNail($imgMainFile['tmp_name'], $main_up_file_name, $main_up_file_name, ROOT.newsUploadPath, $EXT['IMG'], $imgMainFile['size'], "700", "1024", ""); // 섬네일
		}else{
			LIB_FILE::ThumNail($imgMainFile['tmp_name'], $main_up_file_name, $main_up_file_name, ROOT.newsUploadPath, $EXT['IMG'], $imgMainFile['size'], "600", "1024", ""); // 섬네일
		}
