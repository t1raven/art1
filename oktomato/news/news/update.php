...
//postCategory 
if($newsCate1 == '16' ) { //카테고리가 Post 일 경우만 포스트카테고리 값을 처리한다.
	$postCategory = isset($_POST['postCategory']) ? $_POST['postCategory'] : null;
} else {
	$postCategory = null;
}
...
$News->setAttr('post_category', $postCategory);
...
if ($newsCate1 == 15 || $newsCate1 == 16 ){ //15;카드뉴스, 16:포스트는 이미지만 입력된다.
...
// 리센트 이미지 처리 S
	...
	if ($imgRecentFile['name']) {
		...
		LIB_FILE::ThumNail($imgRecentFile['tmp_name'], $recent_up_file_name, $recent_up_file_name, ROOT.newsUploadPath, $EXT['IMG'], MAXFILESIZE, "700", "350", ""); // 섬네일
