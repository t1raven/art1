...
$cv_year = isset($_POST['cv_year']) ? LIB_LIB::getArrayReturn(Xss::chkXSS($_POST['cv_year'])) : null;
$cv_name = isset($_POST['cv_name']) ? LIB_LIB::getArrayReturn(Xss::chkXSS($_POST['cv_name'])) : null;
...
$approval_state = isset($_POST['approval']) ? Xss::chkXSS($_POST['approval']) : null;
...
if (!empty($_POST['artwork_cnt'])) {
	if (is_array($_POST['artwork_cnt'])) {
		foreach($_POST['artwork_cnt'] as $val) {
			if (!empty($val)) {
				if (!is_numeric($val)) {
					Js::Alert('연도별 작품수는 숫자만 입력 가능합니다.');
...
$Artist->setAttr('cv_year', $cv_year);
$Artist->setAttr('cv_name', $cv_name);
...
try {
	if ($_FILES['photo_file']['name']) {
		if (!LIB_FILE::MimeCheck($MIME['IMG'], $_FILES['photo_file']['tmp_name'])) {
			Js::Alert('사용 가능한 파일형식이 아닙니다.('.$_FILES['photo_file']['name'].')');
	...
	if ($_FILES['cv_file']['name']) {
		if (!LIB_FILE::MimeCheck($MIME['PDF'], $_FILES['cv_file']['tmp_name'])) {			
			Js::Alert('사용 가능한 파일형식이 아닙니다.('.$_FILES['cv_file']['name'].')');
