...
$evt_main_img_m = isset($_POST['main_img_m']) ? Xss::chkXSS($_POST['main_img_m']) : null;
$evt_main_img_old_m = isset($_POST['main_img_old_m']) ? Xss::chkXSS($_POST['main_img_old_m']) : null;
...
$Event->setAttr('evt_main_img_m', $evt_main_img_m);
...
try {
	...
	if (!empty($_POST['main_img_m'])) {
		if (!LIB_FILE::moveFile(ROOT.tempUploadPath.$_POST['main_img_m'], ROOT.eventUploadPath.$_POST['main_img_m'])) {
			JS::LocationReplace('파일 이동 오류입니다.', PHP_SELF, 'parent');
			setFree();
			exit();
		}
