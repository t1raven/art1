...
function lstMovMotion(obj){
  ...
  o.each(function(){
    ...
    function mouseEnter(){
      ...
      cont.stop().animate({"margin-top":80},speed,"easeInOutCubic");
    ...
    function mouseLeave(){
      ...
      cont.stop().animate({"margin-top":0},speed,"easeOutCubic");
    ...
    function mouseClick(e){
        ...
        movStartNum = index;
        if(isie7 || isie8){
           itemIeMotion(movStartNum);   

        }else{
          itemMotion(movStartNum);    
        }
    }
...
//**************************************** SOCIAL ****************************************
 ...
 //공유하기
  ...
  function snsZone_2(){
    ...
    var $prev = $obj.find(".prev");
    var $next = $obj.find(".next");
    var boxSize = $pos.find("> article.facebook").outerWidth(true);
    var boxmargin = 0;
    var totalBoxSize = boxSize + boxmargin;
    var exp = 0; // 노출되는 갯수
    var total = 3; //배너 갯수
    ...
    var wideMode = false , mobileMode = false;
    var pageTotal = 0;
    var maskBoxSize = $mask.outerWidth(true);
    var prevMaskBoxSIze = 0;
    

    var startEnd = 0
	var setSnsSlide = setInterval(function(){snsSliding()}, 4000);
	
	function snsSliding(){
		startEnd = 0;
		var $boxIdx =  $pos.find(">.bdrs.firs").index();
		var $boxs = {
			boxA : $pos.find(">.bdrs"),
			box1 : $pos.find(">.bdrs:eq("+($boxIdx%total)+")"),
			box2 : $pos.find(">.bdrs:eq("+(($boxIdx+1)%total)+")"),
			box3 : $pos.find(">.bdrs:eq("+(($boxIdx+2)%total)+")"),	
		};
		var $distan = $boxs.box1.outerWidth(true);
		
		$boxs.box2.siblings().removeClass("firs").end().addClass("firs");
		

		$boxs.box1.addClass("hideAni");
		$boxs.box1.css("transform", "translateX(-"+($distan + ($boxs.box1.index()*$distan))+"px)");
		setTimeout(function(){
			$boxs.box1.removeClass("hideAni");
			$boxs.box1.css("transform","translateX("+((3*$distan) - ($boxs.box1.index()*$distan))+"px)");
			setTimeout(function(){
				$boxs.boxA.addClass("showAni");
				$boxs.box1.css("transform", "translateX("+(($distan*2) - ($boxs.box1.index()*$distan))+"px)");
				$boxs.box2.css("transform", "translateX(-"+($boxs.box2.index()*$distan)+"px)");
				$boxs.box3.css("transform", "translateX("+(($distan)-($boxs.box3.index()*$distan))+"px)");
				setTimeout(function(){
					$boxs.boxA.removeClass("showAni");
				},600);
			},100);
		},600);
	}
	...
	$mask.hover(function(){
		startEnd = 1; // 마우스 들어왔을 때
		clearInterval(setSnsSlide);
	},function(){
		if(startEnd == 1){ // 마우스 나갔을 때
			setSnsSlide = setInterval(function(){snsSliding()}, 4000);
		}
	});
    ...
    resize();
     ...
     function resize(){
        boxSize = $pos.find("> article.facebook").outerWidth(true);
        maskBoxSize = $mask.outerWidth();
        totalBoxSize = boxSize + boxmargin;
        /*con(maskBoxSize,"SNS::::마스크사이즈");
        con(prevMaskBoxSIze,"SNS::::마스크사이즈");*/
	...
	$(window).resize(function(){
		...
		$boxs.box1.removeClass("hideAni");
		$boxs.boxA.removeClass("showAni");
		$boxs.box1.css("transform", "translateX("+(- ($boxs.box1.index()*$distan))+"px)");
		$boxs.box2.css("transform", "translateX("+(($distan)-($boxs.box2.index()*$distan))+"px)");
		$boxs.box3.css("transform", "translateX("+($distan*2 - $boxs.box3.index()*$distan)+"px)");
