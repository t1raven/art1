...
//해시태그  # 을 붙인다.
var hashtagReplace = function(){
    var regExp = /[\{\}\[\]\/?.,;:|\)*~`!^\-+<>\$%&\\\=\(\'\"]/gi;

    $(".inputHashTagReplace").keyup(function(){
        var me = $(this);
        hashTagReplaceFn(me);
    });

    $(".inputHashTagReplace").focus(function(){
        var tag = $(this).val();
        if ( tag.substring(0,1) !== "#") { tag = "#"+tag }  //글자의 맨 앞에 # 을 붙인다.
        $(this).val(tag);
    });
    $(".inputHashTagReplace").blur(function(){
        var tag = $(this).val();
        ...
        tag = hashtagLengthChk(tag);

        if ( tag.substring(tag.length - 1 ,tag.length) === "#") { tag = tag.substring(0, tag.length - 1 ) }  //글자의 맨 뒤의  # 을 없앤다.
        tag = tag.replace(regExp, "");
        $(this).val(tag);
    });
    var hashTagReplaceFn = function (me) {
        var tag = me.val();
        if ( tag.substring(0,1) !== "#") { tag = "#"+tag }  //글자의 맨 앞에 # 을 붙인다.
        if ( ~tag.indexOf(' ') || ~tag.indexOf(',') || ~tag.indexOf('##')) { tag = tag.replace(/ /gi,"#").replace(/##/gi,"#").replace(",","#"); }
        tag = tag.replace(regExp, "");
        me.val(tag);
    };
    var hashtagLengthChk = function( tag) {
        var aTag = tag.split('#');

        for (var i = 1; i < aTag.length; i++) {
            if (aTag[i].length < 2) {
                alert("["+aTag[i] +"]  해시태그는 2자 이상 입력하셔야 합니다.");
                tag = tag.replace("#"+aTag[i],"");
            }
        }
        if ( tag.substring(0,1) !== "#") { tag = "#"+tag }  //글자의 맨 앞에 # 을 붙인다.
