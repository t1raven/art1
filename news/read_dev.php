...
try {
	...
	// 페이스북 쉐어를 위한 메타 데이터 S
	// /inc/top.php 에서 처리
	$ogDescription = LIB_LIB::CutString($News->getNewsPragraphOne($dbh), 50) ; //첫번째 단락을 가져온다. (페이스북 쉐어를 위한 데이터)
	$ogDescription = ( !empty($ogDescription) ) ? preg_replace('/\r\n|\r|\n/',' ', $ogDescription ) : null  ;   //개행문자를 없앤다.
	$ogTitle = $News->attr['news_title'];
	$ogImage = HOME.newsUploadPath. $News->attr['news_main_up_file_name'];
	// 페이스북 쉐어를 위한 메타 데이터 E
