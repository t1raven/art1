	...
	<div class="container_inner pin">
		...
		<section class="articovery_pin_main">
			<h1 class="h">
				...
				<a href="#">VIEW MORE (<?php echo number_format($worksTotalCount); ?>)</a>
			</h1>
...
function bpop(id){
	$("#popup_prd").find("div > img").attr("src", "/images/articovery/noimg.png");
	...
	$.ajax({
		...
		success:function(data){
			...
			//$("#popup_prd").html(data);
			if(data.result == 1){
					$("#popup_prd").find("div > img").attr("src", data.works_img);
