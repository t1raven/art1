...
<section id="container_sub" class="artipin">
	...
	<div class="container_inner pin">
		...
		<?php endif; ?>
			<div class="pageing">
				<button class="prev"><img src="/images/articovery/btn_prev.png" alt=""></button>
				<button class="next"><img src="/images/articovery/btn_next.png" alt=""></button>
			</div>
...
$(function(){<?php if ($_SESSION['user_idx']): ?>loginPopClose();<?php endif; ?>});
			...
			if(data.result == 1){
				if(data.contact == 0){
					if(data.pin_cnt == 0){
						...
						if(flag == "L"){
							$("#a-pin-"+idx).attr("onclick", "layerPopup3.open('index.php','popup_pin_cancel','',{'idx':'"+idx+"','at':'cancel', 'flag':'L'}); return false;");
						}else{
							$("#popup_prd").find("div.head > div > div > button.pin").addClass("on").attr("onclick", "layerPopup3.open('index.php','popup_pin_cancel','',{'idx':'"+idx+"','at':'cancel'}); return false;");
							$("#a-pin-"+idx).attr("onclick", "layerPopup3.open('index.php','popup_pin_cancel','',{'idx':'"+idx+"','at':'cancel', 'flag':'L'}); return false;");
						}
					}
...
function setMyPinCancel(idx, flag){
	$.ajax({
		url:'index.php',
		type:'post',
		...
		success:function(data){
			if(data.result == 1){
				if(data.pin_cnt == 0){
					if(flag == "L"){
						$("#a-pin-"+idx).attr("onclick", "layerPopup3.open('index.php','popup_alert1','',{'idx':'"+idx+"','at':'pin', 'flag':'L'}); return false;");
					}else{
