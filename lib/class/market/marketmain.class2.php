...
*/
	...
	function getGoodsListInfo($dbh) {
		try {
			...
			$goods_name = '%'.$goods_name.'%';
			$sql = 'SELECT a.goods_idx, a.goods_name, a.artist_idx, b.artist_name FROM goods as a inner join artist_info as b on a.artist_idx = b.artist_idx WHERE a.goods_name like :goods_name and del_state = :del_state';
			$stmt = $dbh->prepare($sql);
			$stmt->bindParam(':goods_name', $goods_name, PDO::PARAM_STR, 200);
