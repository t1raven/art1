...
*/
			...
			$stmt->bindParam(':goods_rental_price', $goods_rental_price, PDO::PARAM_INT, 11);
...
*/
	...
	function getEdit($dbh) {
		try {
			...
			$sql = 'SELECT
									...
									a.goods_width, a.goods_depth, a.goods_height, a.goods_sell_price, a.goods_rental_price, a.goods_description, a.goods_material, a.goods_display,
	...
	// 아트 상품 목록 가져오기
	function getMainGoodsList($dbh) {
		try {
			$list = null;
			$category_idx = 2;
			$del_state = 'N';
			$sql = 'SELECT goods_idx, goods_name, goods_sell_price, goods_list_img FROM goods WHERE category_idx = :category_idx AND del_state = :del_state ORDER BY goods_idx DESC';
			$stmt = $dbh->prepare($sql);
			$stmt->bindParam(':category_idx', $category_idx, PDO::PARAM_INT, 1);
			$stmt->bindParam(':del_state', $del_state, PDO::PARAM_STR, 1);
	...
	function getRelationGoodsImg($dbh) {
		try {
			$sql = 'SELECT goods_idx, goods_list_img FROM goods WHERE category_idx = 2 ORDER BY goods_idx DESC';
			$stmt = $dbh->prepare($sql);
			if ($stmt->execute() && $stmt->rowCount()) {
				return $stmt->fetchAll(PDO::FETCH_ASSOC);
			}
