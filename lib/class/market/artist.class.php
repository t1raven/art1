...
class Artist
{
	...
	function insert($dbh) {
		try {
			...
			$cv_year = $this->getAddslashes('cv_year');
			$cv_name = $this->getAddslashes('cv_name');
			...
			$stmt->bindParam(':cv_year', $cv_year, PDO::PARAM_STR, 15);
			$stmt->bindParam(':cv_name', $cv_name, PDO::PARAM_STR, 300);
	...
	function delete($dbh) {
		try {
			...
			if ($bTransaction = self::attachFileDelete($dbh, $artist_idx)) {
				$sql = 'DELETE FROM artist_info WHERE artist_idx = :artist_idx';
				$stmt = $dbh->prepare($sql);
				$stmt->bindParam(':artist_idx', $artist_idx, PDO::PARAM_INT);
			...
			*/
			...
			if ($bTransaction = self::deleteGoods($dbh, $artist_idx)) {
				$sql = 'UPDATE artist_info set del_state = :del_state WHERE artist_idx = :artist_idx';
				$stmt = $dbh->prepare($sql);
	...
	function getEdit($dbh) {
		try {
			...
			//echo $artist_idx;
			$sql = 'SELECT
									artist_idx, artist_name, artist_en_name, photo_up_file_name, photo_ori_file_name, cv_up_file_name, cv_ori_file_name, artist_job, artist_birthday, artist_genre, artist_mobile, artist_email, education_year, education_name, award_year, award_name, private_year, private_name, team_year, team_name,  cv_year, cv_name, major_work_idx, artist_greeting, annual_artwork_year, annual_artwork_cnt, homepage_url, blog_url, facebook_url, sns_1_name, sns_1_url, sns_2_name, sns_2_url, approval_state,
	...
	function deleteAttachAjax($dbh, $attach) {
		...
		if ($attach === 'photo') {
			$sql = 'UPDATE artist_info SET photo_up_file_name = :photo_up_file_name, photo_ori_file_name = :photo_ori_file_name WHERE artist_idx = :artist_idx';
			$stmt = $dbh->prepare($sql);
			$stmt->bindParam(':photo_up_file_name', $photo_up_file_name, PDO::PARAM_STR, 25);
			$stmt->bindParam(':photo_ori_file_name', $photo_ori_file_name, PDO::PARAM_STR, 255);
			$stmt->bindParam(':artist_idx', $artist_idx, PDO::PARAM_INT, 11);
		...
		elseif ($attach === 'cv') {
			$sql = 'UPDATE artist_info SET cv_up_file_name = :cv_up_file_name, cv_ori_file_name = :cv_ori_file_name WHERE artist_idx = :artist_idx';
			$stmt = $dbh->prepare($sql);
			$stmt->bindParam(':cv_up_file_name', $cv_up_file_name, PDO::PARAM_STR, 25);
			$stmt->bindParam(':cv_ori_file_name', $cv_ori_file_name, PDO::PARAM_STR, 255);
			$stmt->bindParam(':artist_idx', $artist_idx, PDO::PARAM_INT, 11);
	...
	// 작품 삭제
	function deleteGoods($dbh, $artist_idx) {
		try {
			$bTransaction = true;
			...
			$sql = 'SELECT goods_idx FROM goods WHERE artist_idx = :artist_idx';
			$stmt = $dbh->prepare($sql);
			$stmt->bindParam(':artist_idx', $artist_idx, PDO::PARAM_INT, 11);

			if ($stmt->execute()) {
				$aGoodsIdx = $stmt->fetchAll(PDO::FETCH_ASSOC);

				foreach($aGoodsIdx as $row) {
					$sql = 'DELETE FROM basket WHERE goods_idx = :goods_idx';
					$stmt = $dbh->prepare($sql);
					...
					if ($stmt->execute()) {
						...
						$sql = 'DELETE FROM goods_wish WHERE goods_idx = :goods_idx';
						$stmt = $dbh->prepare($sql);
						$stmt->bindParam(':goods_idx', $row['goods_idx'], PDO::PARAM_INT, 11);
				...
				if ($bTransaction) {
					$sql = 'UPDATE goods SET del_state = :del_state WHERE artist_idx = :artist_idx';
					$stmt = $dbh->prepare($sql);
					$stmt->bindParam(':del_state', $del_state, PDO::PARAM_STR, 1);
					$stmt->bindParam(':artist_idx', $artist_idx, PDO::PARAM_INT, 11);
