...
class Recommend
{
	...
	function insert($dbh) {
		try {
			...
			$sql = 'INSERT INTO recommend (referee, img_up_file_name, img_ori_file_name) VALUES(:referee, :img_up_file_name, :img_ori_file_name)';
			$stmt = $dbh->prepare($sql);
			$stmt->bindParam(':referee', $referee, PDO::PARAM_STR, 50);
			$stmt->bindParam(':img_up_file_name', $img_up_file_name, PDO::PARAM_STR, 25);
			$stmt->bindParam(':img_ori_file_name', $img_ori_file_name, PDO::PARAM_STR, 255);
	...
	function update($dbh) {
		try {
			...
			$img_up_file_name = $this->getAddslashes('img_up_file_name');
			$img_ori_file_name = $this->getAddslashes('img_ori_file_name');
			$sql = 'UPDATE recommend SET referee = :referee, img_up_file_name = :img_up_file_name, img_ori_file_name = :img_ori_file_name WHERE recommend_idx = :recommend_idx';
			$stmt = $dbh->prepare($sql);
			$stmt->bindParam(':referee', $referee, PDO::PARAM_STR, 50);
			$stmt->bindParam(':img_up_file_name', $img_up_file_name, PDO::PARAM_STR, 25);
			$stmt->bindParam(':img_ori_file_name', $img_ori_file_name, PDO::PARAM_STR, 255);
	...
	function getList($dbh) {
		try {
			$del_state = 'N';
			$sql = 'SELECT recommend_idx, referee, img_up_file_name, img_ori_file_name FROM recommend WHERE del_state = :del_state';
			$stmt = $dbh->prepare($sql);
			$stmt->bindParam(':del_state', $del_state, PDO::PARAM_STR, 1);
	...
	function getFileInfo($dbh, $recommend_idx) {
		try {
			$sql = 'SELECT recommend_idx, img_up_file_name, img_ori_file_name FROM recommend WHERE recommend_idx = :recommend_idx';
			$stmt = $dbh->prepare($sql);
	...
	function deleteAttachAjax($dbh, $attach) {
		...
		if ($attach === 'img') {
			$sql = 'UPDATE recommend SET img_up_file_name = :img_up_file_name, img_ori_file_name = :img_ori_file_name FROM recommend WHERE recommend_idx = :recommend_idx';
			$stmt = $dbh->prepare($sql);
			$stmt->bindParam(':img_up_file_name', $img_up_file_name, PDO::PARAM_STR, 25);
			$stmt->bindParam(':img_ori_file_name', $img_ori_file_name, PDO::PARAM_STR, 255);
			$stmt->bindParam(':recommend_idx', $recommend_idx, PDO::PARAM_INT, 11);
