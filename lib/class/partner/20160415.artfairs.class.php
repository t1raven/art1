...
class Artfairs
{
	...
	function getList($dbh) {
		try {
			...
			$subQuery = '';
			$start = ($pg - 1) * $sz;
			$query = ' SELECT fairIdx, fairTitle, fairTitleEn, createDate FROM TB_GALLERY_ARTFAIR ';

			if (!empty($nm)) {
				$subQuery = ' AND (fairTitle LIKE :fairTitle) ';
				$fairTitle = $nm."%";
			}
			...
			$sql = $query.' WHERE  galleryIdx = :galleryIdx '.$subQuery.' ORDER BY fairIdx DESC LIMIT :start, :end';
			$stmt = $dbh->prepare($sql);
			$stmt->bindParam(':galleryIdx', $galleryIdx, PDO::PARAM_INT, 10);
			...
			if ($stmt->execute() && $stmt->rowCount()) {
				...
				$sql = 'SELECT COUNT(fairIdx) FROM TB_GALLERY_ARTFAIR WHERE galleryIdx = :galleryIdx '.$subQuery;
				$stmt = $dbh->prepare($sql);
	...
	function getEdit($dbh) {
		try {
			...
			$sql = ' SELECT
							...
							endDate,
							link
						FROM TB_GALLERY_ARTFAIR
						WHERE fairIdx = :fairIdx AND galleryIdx = :galleryIdx';
			$stmt = $dbh->prepare($sql);
			$stmt->bindParam(':fairIdx', $fairIdx, PDO::PARAM_INT, 10);
			$stmt->bindParam(':galleryIdx', $galleryIdx, PDO::PARAM_INT, 10);
		...
		try {
			...
			if (!empty($sw)) {
				...
				$sql = 'SELECT
								...
								a.endDate,
								a.link,
								(SELECT fileIdx FROM TB_GALLERY_ARTFAIR_FILE WHERE fairIdx = a.fairIdx LIMIT 0, 1) AS fileIdx,
								(SELECT upfileName FROM TB_GALLERY_ARTFAIR_FILE WHERE fairIdx = a.fairIdx LIMIT 0, 1) AS upfileName
							FROM TB_GALLERY_ARTFAIR_POOL AS a
							WHERE fairTitle LIKE :fairTitle';
				$stmt = $dbh->prepare($sql);
				$stmt->bindParam(':fairTitle', $sw, PDO::PARAM_STR, 50);
