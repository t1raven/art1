...
class Artworks
{
	...
	function insert($dbh) {
		try {
			...
			$worksName = strip_tags($this->attr['worksName']);
			$worksNameEn = strip_tags($this->attr['worksNameEn']);
			...
			$sort = $this->getMaxSort($dbh, $galleryIdx);
			...
			$stmt->bindParam(':sort', $sort, PDO::PARAM_INT, 10);
	...
	function getList($dbh) {
		try {
			...
			$subQuery = '';
			$start = ($pg - 1) * $sz;
			$query = ' SELECT worksIdx, artistName, artistNameEn, worksName, worksNameEn, sort, createDate FROM TB_GALLERY_WORKS ';
			...
			$sql = $query.' WHERE  galleryIdx = :galleryIdx '.$subQuery.' ORDER BY sort DESC, worksIdx DESC LIMIT :start, :end';
			$stmt = $dbh->prepare($sql);
			$stmt->bindParam(':galleryIdx', $galleryIdx, PDO::PARAM_INT, 10);
	...
	// 순서 지정
	function setSorts($dbh) {
		try {
			$worksIdxs = $this->getAddslashes('widxs');
			$sorts = $this->getAddslashes('sorts');
			//print_r($worksIdxs);

			if (is_array($worksIdxs) && is_array($sorts)) {
				$bTransaction = false;
				$dbh->beginTransaction();

				foreach($worksIdxs as $key => $worksIdx) {
					$sql = 'UPDATE TB_GALLERY_WORKS SET
									sort = :sort
								WHERE worksIdx = :worksIdx';
					$stmt = $dbh->prepare($sql);
					$stmt->bindParam(':worksIdx', $worksIdx, PDO::PARAM_INT, 10);
					$stmt->bindParam(':sort', trim($sorts[$key]), PDO::PARAM_INT, 10);
	...
	function getMaxSort($dbh, $galleryIdx) {
		try {
			if (!empty($galleryIdx)) {
				$sql = 'SELECT MAX(sort) AS sort FROM TB_GALLERY_WORKS WHERE galleryIdx = :galleryIdx';
				$stmt = $dbh->prepare($sql);
				$stmt->bindParam(':galleryIdx', $galleryIdx, PDO::PARAM_INT, 10);
				if ($stmt->execute() && $stmt->rowCount()) {
