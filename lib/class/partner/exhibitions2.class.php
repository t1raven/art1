...
class Exhibition
{
			...
			if ($stmt->execute()) {
				...
				if (is_array($artIdxs)) {
					foreach($artIdxs as $key => $val) {
						if (!empty($val)) {
							$sql = 'INSERT INTO TB_GALLERY_EXHIBITION_ARTIST SET
											exhibitionIdx = :exhibitionIdx,
											artistIdx = :artistIdx,
											artistName = :artistName,
											artistNameEn = :artistNameEn,
	...
	function update($dbh) {
		try {
			...
			$aExhiArtistIdx = $this->getAddslashes('exhiArtistIdx');
			$artIdxs = $this->getAddslashes('artIdxs');
			$artNames = $this->getAddslashes('artNames');
			$artNamesEn = $this->getAddslashes('artNamesEn');
			...
			if ($stmt->execute()) {
				...
				if (is_array($artIdxs)) {
					foreach($artIdxs as $key => $val) {
						if (!empty($aExhiArtistIdx[$key])) {
							$sql = 'UPDATE TB_GALLERY_EXHIBITION_ARTIST SET
											exhibitionIdx = :exhibitionIdx,
											artistIdx = :artistIdx,
											artistName = :artistName,
											artistNameEn = :artistNameEn,
											ipAddr = :ipAddr,
											modifyDate = :modifyDate
										WHERE exhiArtistIdx = :exhiArtistIdx';
							$stmt = $dbh->prepare($sql);
							$stmt->bindParam(':exhibitionIdx', $exhibitionIdx, PDO::PARAM_INT, 10);
							$stmt->bindParam(':artistIdx', $val, PDO::PARAM_INT, 10);
							$stmt->bindParam(':artistName', $artNames[$key], PDO::PARAM_STR, 50);
							...
							$stmt->bindParam(':ipAddr', $ipAddr, PDO::PARAM_INT, 10);
							$stmt->bindParam(':modifyDate', $date, PDO::PARAM_STR);
							$stmt->bindParam(':exhiArtistIdx', $aExhiArtistIdx[$key], PDO::PARAM_INT, 10);
	...
	function deleteArtistAjax($dbh, $attach) {
		try {
			$exhiArtistIdx = $this->getAddslashes('eaidx');
			$exhibitionIdx = $this->getAddslashes('idx');
			...
			if (empty($attach)) {
				$sql = 'DELETE FROM TB_GALLERY_EXHIBITION_ARTIST WHERE exhiArtistIdx = :exhiArtistIdx AND exhibitionIdx = :exhibitionIdx';
				$stmt = $dbh->prepare($sql);
				$stmt->bindParam(':exhiArtistIdx', $exhiArtistIdx, PDO::PARAM_INT, 10);
				$stmt->bindParam(':exhibitionIdx', $exhibitionIdx, PDO::PARAM_INT, 10);
	...
	function getList($dbh) {
		try {
			...
			$query = ' SELECT
								a.exhibitionIdx,
								(SELECT GROUP_CONCAT(artistName) AS artistName FROM TB_GALLERY_EXHIBITION_ARTIST WHERE exhibitionIdx = a.exhibitionIdx) AS artistName,
								a.exhibitionTitle,
								a.exhibitionTitleEn,
								a.createDate
			...
			if (!empty($exh) && !empty($nm)) {
				if ($st === 0) {
					$subQuery = ' AND (a.exhibitionTitle LIKE :exhibitionTitle AND (SELECT GROUP_CONCAT(artistName) AS artistName FROM TB_GALLERY_EXHIBITION_ARTIST WHERE exhibitionIdx = a.exhibitionIdx) LIKE :artistName) ';
				}
				else {
					$subQuery = ' AND (a.exhibitionTitle LIKE :exhibitionTitle OR (SELECT GROUP_CONCAT(artistName) AS artistName FROM TB_GALLERY_EXHIBITION_ARTIST WHERE exhibitionIdx = a.exhibitionIdx) LIKE :artistName) ';
				}
				$exhibitionTitle = $exh."%";
				$artistName = "%".$nm."%";
			}
			else if (!empty($exh) && empty($nm)) {
				$subQuery = ' AND a.exhibitionTitle LIKE :exhibitionTitle ';
				$exhibitionTitle = $exh."%";
			}
			else if (empty($exh) && !empty($nm)) {
				$subQuery = ' AND (SELECT GROUP_CONCAT(artistName) AS artistName FROM TB_GALLERY_EXHIBITION_ARTIST WHERE exhibitionIdx = a.exhibitionIdx) LIKE :artistName ';
				$artistName = "%".$nm."%";
			}
			...
			$sql = $query.' WHERE  a.galleryIdx = :galleryIdx '.$subQuery.' ORDER BY a.exhibitionIdx DESC LIMIT :start, :end';
			//echo $sql;
			$stmt = $dbh->prepare($sql);
			$stmt->bindParam(':galleryIdx', $galleryIdx, PDO::PARAM_INT, 10);
			...
			if ($stmt->execute() && $stmt->rowCount()) {
				...
				$sql = 'SELECT COUNT(exhibitionIdx) FROM TB_GALLERY_EXHIBITION AS a WHERE a.galleryIdx = :galleryIdx '.$subQuery;
				$stmt = $dbh->prepare($sql);
	...
	function getSelectedArtists($dbh) {
		try {
			$exhibitionIdx = $this->getAddslashes('idx');
			$sql = 'SELECT exhiArtistIdx, artistIdx, artistName, artistNameEn  FROM TB_GALLERY_EXHIBITION_ARTIST WHERE exhibitionIdx = :exhibitionIdx';
			$stmt = $dbh->prepare($sql);
			$stmt->bindParam(':exhibitionIdx', $exhibitionIdx, PDO::PARAM_INT, 10);
