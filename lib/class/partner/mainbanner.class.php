...
class Mainbanner
{
	...
	function update($dbh) {
		try {
			...
			$aBannerTitle = $this->getAddslashes('bannerTitle');
			$aBannerCaption = $this->getAddslashes('bannerCaption');
			...
			foreach($aBannerIdx as $key => $bannerIdx) {
				$sql = 'UPDATE TB_GALLERY_MAIN_BANNER SET
				...
				$stmt->bindParam(':bannerTitle', $aBannerTitle[$key], PDO::PARAM_STR,255);
				$stmt->bindParam(':bannerCaption', $aBannerCaption[$key], PDO::PARAM_STR, 255);
	...
	function getFileInfo($dbh) {
		try {
			$bannerIdx = $this->getAddslashes('idx');
			$sql = 'SELECT upfileName FROM TB_GALLERY_MAIN_BANNER WHERE bannerIdx = :bannerIdx';
			$stmt = $dbh->prepare($sql);
			$stmt->bindParam(':bannerIdx', $bannerIdx, PDO::PARAM_INT, 10);
