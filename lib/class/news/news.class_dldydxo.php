...
class News
{
				...
				foreach ($arr_ImgOrVideo as $value) {
					...
					if ($arr_this_value[$i]) { //유효한 입력 값이 있으면 처리
						...
						if ($arr_paragraph_idx[$i]){ //저장된 내용 수정
							...
							$stmt = $dbh->prepare($sql);
							$stmt->bindParam(':new_paragraph_content',$new_paragraph_content,	PDO::PARAM_INT);
							$stmt->bindParam(':img_or_video',$img_or_video,										PDO::PARAM_STR, 30);
							$stmt->bindParam(':ori_file_name',$ori_file_name,										PDO::PARAM_STR, 30);
							$stmt->bindParam(':up_file_name',$up_file_name,										PDO::PARAM_STR, 30);
							$stmt->bindParam(':img_comment',$img_comment,										PDO::PARAM_STR, 30);
	...
	function delete_main_file($dbh){ //메인 개별 파일 삭제
		try {
			$news_idx = $this->getAddslashes('news_idx');
			$bTransaction = false;
			$dbh->beginTransaction();
...
*/
	...
	function getFrontList($dbh) {
		try {
			...
			//echo(" CALL up_news_front_list ( $page, $sz, $cate, $word, $start_row, $recent_status ) <br>");
			...
			if ($stmt->execute() && $stmt->rowCount()) {
				...
				$sql = 'SELECT news_idx FROM news_main 
							WHERE 
								...
								AND news_category_idx = :news_category_idx 
								AND  display_status=\'Y\' 
							ORDER BY news_idx DESC limit 1 ';
				$stmt = $dbh->prepare($sql);
				$stmt->bindParam(':news_idx', $news_idx,											 PDO::PARAM_INT);
				$stmt->bindParam(':news_category_idx', $news_category_idx,			 PDO::PARAM_STR, 20);
				$stmt->execute();
	...
	function getFrontReadMainNews($dbh) {
		try {
			...
			$sql = 'select * from view_news_paragraph_row where news_category_idx = 1 and  display_status =\'Y\' ORDER BY news_idx DESC LIMIT 1';
			$stmt = $dbh->prepare($sql);
			$stmt->bindParam(':news_idx', $news_idx, PDO::PARAM_INT);
			//$stmt->execute();
