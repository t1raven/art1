...
class LIB_LIB
{
	...
	*/
	...
	/**
	주간날짜 구함
	param : $inDay = "2017-03-04 00:00:00" 날짜 
	return :  array //$inDay 가 속한 주간의 월요일 날짜, 다음 일요일 날짜  
	*/
	public static function weeklyDay($inDay) {
		if(!empty($inDay)) {
			$m_date = explode("-",substr($inDay, 0, 10) ); 
			$inputYear = $m_date[0]; 
			$inputMonth = $m_date[1]; 
			$inputDay = $m_date[2]; 
			$weekMktime = date("D", mktime(0, 0, 0, $m_date[1], $m_date[2], $m_date[0]));
	...
	작품 사이즈 리턴
	*/
	public static function getGoodsSize($depth, $width, $height) {
				$artWorkSize = null;
				$width = !empty($width) ? str_replace('.0','',$width) : null;
