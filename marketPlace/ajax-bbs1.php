...
$artWorkTotalCount = (int)$Marketmain->getArtWorkCount($dbh);
$j = 0;

for ($i = 0; $i < $artWorkTotalCount; $i+=20) {
	$aValue[$j] = $i;
	$j++;
}
		...
		<section class="newsBox <?php echo $aEngMedium[$row['goods_medium']];?> <?php echo $aEngSubject[$row['goods_subject']];?> <?php echo $aEngSize[$row['goods_size']];?> <?php echo $aEngColor[$row['goods_color']];?> <?php echo $price;?>" id="isto<?php echo $row['goods_idx']; ?>">
			<div class="newsBox_inner">
				<div class="Boximg">
					...
					<a href="#" onclick="goods='<?php echo $row['goods_idx']; ?>'; marketViewMotion(); return false;"><img src="<?php echo goodsThumbImgUploadPath, $row['goods_list_img'];?>" alt="" /></a>
				</div>
				<div class="news_cont">
					<!--h1><a href="artwork_view.php?goods=<?php echo $row['goods_idx'];?>"><?php echo $row['goods_name'];?></a></h1-->
					<h1><a href="#" onclick="goods='<?php echo $row['goods_idx']; ?>'; marketViewMotion(); return false;"><?php echo $row['goods_name'];?></a></h1>
					<div class="text_cont">
